...
<div class="p-4 sm:ml-64">
    <div class="px-10 py-6 mt-1 flex flex-col md:flex-row items-center h-auto rounded-md bg-white border border-gray-200 p-5">
        <h1 class="text-2xl font-bold text-left mb-4 md:mb-0 md:w-auto md:flex-1">Data Tahun Ajaran</h1>
        <x-breadcrumb parent="Data Master" item="Tahun Ajaran" />
    </div>
    ...
    <div class="px-10 py-8 mt-3 p-5 rounded-md bg-white border border-gray-200">
        ...
        <div class="flex justify-between mb-4 flex-wrap">
            ...
            <div class="flex justify-end mb-4 flex-grow">
                <form action="{{ route('tahun_ajaran.search') }}" method="GET" class="max-w-md  w-full" id="search-form">
                    <div class="relative">
                        ...
                        <input type="search" id="search-input" name="search"
                            ...
                            placeholder="Cari data tahun ajaran disini"
        ...
        <div id="crud-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-full max-h-full bg-black bg-opacity-45">
            <div class="relative p-4 w-full max-w-md max-h-full">
                ...
                <div class="relative bg-white rounded-lg shadow-sm">
                    ...
                    <!-- Modal body -->
                    <form action="{{ route('tahun_ajaran.store') }}" method="POST" class="p-4 md:p-5">
                        @csrf
                        <div class="grid gap-4 mb-4 grid-cols-2">
                            <div class="col-span-2">
                                <label for="tahun_ajaran" class="block mb-2 text-sm font-medium text-gray-900">Tahun Ajaran</label>
                                <input type="text" name="tahun_ajaran" id="tahun_ajaran" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="2022/2023" required />
                            </div>
                        </div>
        ...
        @if($tahunAjaran->isEmpty())
            <div class="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded relative text-center" role="alert">
                ...
                <span class="block sm:inline">Tidak ada data tahun ajaran yang bisa ditampilkan. Silakan tambahkan data tahun ajaran.</span>
            </div>
        @else
                ...
                <table class="mb-4 table-auto w-full border-collapse border border-gray-200">
                    <thead class="bg-gray-100">
                        <tr class="text-center">
                            ...
                            <th class="w-9/12 border border-gray-300 px-4 py-2 whitespace-nowrap">Tahun Ajaran</th>
                            <th class="w-2/12 border border-gray-300 px-4 py-2">Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($tahunAjaran as $tahun)
                            <tr class="hover:bg-gray-50">
                                <td class="border border-gray-300 px-4 py-2 text-center">{{ $loop->iteration + ($tahunAjaran->currentPage() - 1) * $tahunAjaran->perPage() }}</td>
                                <td class="border border-gray-300 px-4 py-2">{{ $tahun->tahun_ajaran }}</td>
                                <td class="border border-gray-300 px-4 py-2 text-center">
                                    <div class="flex justify-center space-x-2">
                                        <button data-modal-target="editModal-{{ $tahun->id }}" data-modal-toggle="editModal-{{ $tahun->id }}" class="text-sm flex items-center justify-center w-full px-3 py-2 bg-yellow-400 text-white rounded-lg hover:bg-yellow-600 transition duration-200">
                                        ...
                                        <button data-modal-target="popup-modal-{{ $tahun->id }}" data-modal-toggle="popup-modal-{{ $tahun->id }}"
                                    ...
                                    <!-- Modal Edit -->
                                    <div id="editModal-{{ $tahun->id }}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-full max-h-full bg-black bg-opacity-45">
                                        <div class="relative p-4 w-full max-w-md max-h-full">
                                            <!-- Modal content -->
                                            <div class="relative bg-white rounded-lg shadow-sm">
                                                <!-- Modal header -->
                                                <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t border-gray-200">
                                                    ...
                                                    <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center" data-modal-toggle="editModal-{{ $tahun->id }}">
                                                        <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                                                        </svg>
                                                        <span class="sr-only">Close modal</span>
                                                ...
                                                <!-- Modal body -->
                                                <form action="{{ route('tahun_ajaran.update', $tahun->id) }}" method="POST" class="p-4 md:p-5">
                                                    @csrf
                                                    @method('PUT')
                                                    <div class="grid gap-4 mb-4 grid-cols-2">
                                                        <div class="col-span-2">
                                                            <label for="edit_tahun_ajaran" class="block mb-2 text-sm font-medium text-gray-900">Tahun Ajaran</label>
                                                            <input type="text" name="tahun_ajaran" id="edit_tahun_ajaran" value="{{ $tahun->tahun_ajaran }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="D3 Teknik Informatika" required />
                                                        </div>
                                                    </div>
                                                    <div class="flex justify-end space-x-2">
                                                        <button type="button" class="text-white inline-flex items-center bg-gray-600 hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center" data-modal-toggle="editModal-{{ $tahun->id }}">
                                                            <svg class="me-2 w-2 h-2" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                                                            </svg>
                                                            Batal
                                        ...
                                        <!-- Modal Hapus-->
                                    <div id="popup-modal-{{ $tahun->id }}" tabindex="-1"
                                        ...
                                        <div class="relative p-4 w-full max-w-md max-h-full">
                                            <div class="relative bg-white rounded-lg shadow-sm">
                                                <button type="button" class="absolute top-3 end-2.5 text-gray-400 bg-transparent
                                                    ...
                                                    inline-flex justify-center items-center" data-modal-hide="popup-modal-{{ $tahun->id }}">
                                                ...
                                                <div class="p-4 md:p-5 text-center">
                                                    ...
                                                    <h3 class="mb-5 text-lg font-normal text-gray-500">
                                                        Apakah anda yakin ingin menghapus data tahun ajaran ini?
                                                    </h3>
                                                    <form id="delete-form-{{ $tahun->id }}" action="{{ route('tahun_ajaran.destroy', $tahun->id) }}" method="POST" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    ...
                                                    <button data-modal-hide="popup-modal-{{ $tahun->id }}" type="button"
            ...
            <nav aria-label="Page navigation example">
                {{ $tahunAjaran->links() }} <!-- Ini akan menghasilkan pagination -->
            </nav>
        @endif
    </div>
</div>
...
{{-- <script>
    function openEditModal(id, tahunAjaran) {
